...
namespace KompasCore.Effects
{
    ...
    public class MovementRestriction
    {
        ...
        /*{
            if (!ValidIndices(x, y)) return false;
            foreach(var r in normalRestrictions)
            {
                if(!RestrictionValid(r, x, y, isSwapTarget: isSwapTarget, byEffect: false))
                {
                    Debug.LogError($"not allowed to normal move to {x}, {y} because {r}");
                    return false;
                }
            }
            return true;
